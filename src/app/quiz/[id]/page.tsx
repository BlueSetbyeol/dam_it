...
export default function Quiz() {
  ...
  const randomQuestion = Math.floor(Math.random() * questionsAvailable.length);
  console.log(randomQuestion);

  const firstQuestion = objects_to_sort.find(
    (sort) =>
      sort.id === Number(questionsAvailable[randomQuestion].object_to_sort_id)
  );

  const secondQuestion = objects_to_repair.find(
    (sort) =>
      sort.id === Number(questionsAvailable[randomQuestion].object_to_repair_id)
  );

  return (
    <section>
      <p>{firstQuestion?.mission}</p>
      {firstQuestion?.answers.map((object, index) => (
